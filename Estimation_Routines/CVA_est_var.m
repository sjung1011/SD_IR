function [CVA_w] = CVA_est_var(Y, settings,ei_var,ei_bcvar,ei_bvar,responseV)
% make as an additional case in estimation part in run_dfm.m
% leave-h-out cross validation averaging (CVA)
% all vars have the same lag orders 
...
for h=0:IRF_hor-1
    ...
    if h>0
        ...
        for t=h:1:T-h+1
            ...
            % Adjust residuals for BC
            ehat_slec = ei_bcvar(t-h+1:t+h-1,:);
            ehat_h_slec = inv(eye_mat-Pth)*ehat_slec;
...
for h = 0:IRF_hor-1
    field_name = sprintf('h%d', h);
    ee_all.(field_name) = [ee_ls.(field_name), ee_bc.(field_name), ee_bvar.(field_name)]; % T by m
