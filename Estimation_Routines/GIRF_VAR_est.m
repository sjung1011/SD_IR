function [irf, band_lo, band_hi, Res] = GIRF_VAR_est(Y, settings,responseV,recurShock,delta_in)
% -------------------------------------------------------------
%  LS VAR(p) paths 
...
for b = 1:B
    ...
    % 4.3  propagate both paths with companion form (in fact the same paths are generated) 
    for h = 0:H
        yB = sel*stB + epsB(h+1,:)';
        yS = sel*stS + epsS(h+1,:)';
