function [Bc,Br,Bx,By,Sigma,Sxx,w, y, X, x, Res,V] = LP_emp(Y,recurShock,respV,nlags,nhorizon)
% data for LP w/o contmp terms
nv = size(Y,2);
% nT = size(Y,1);
...
X = [ones(size(x,1),1), x, w]; % w/o contmp terms
...
Bc = Beta(1); % constant
Br = []; % no contemperaneous control
Bx = Beta(2); % as contmp shock variable put next to constant regardless of ff, uncertainty shock
By = Beta(3:end); % lagged controls
